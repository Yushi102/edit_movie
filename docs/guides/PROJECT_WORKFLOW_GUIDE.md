# 動画編集AI - プロジェクト全体の流れ（カット選択版）

## 🎯 プロジェクトの目的
動画から自動的に**最適なカット位置**を予測し、Premiere Pro用のXMLを生成する

**現在の開発フォーカス**: カット選択（Cut Selection）に特化
- ✅ カット選択モデル: 動作中（平均F1: 42.30%、最良: 49.42%）
- ⚠️ グラフィック配置・テロップ生成: 精度が低いため今後の課題

---

## 📊 全体フロー図

```
┌─────────────────────────────────────────────────────────────────┐
│                    1. データ準備フェーズ                          │
└─────────────────────────────────────────────────────────────────┘
                                  ↓
    ┌──────────────────────────────────────────────────────┐
    │ 1-1. 編集済み動画 + Premiere Pro XMLを用意           │
    │      (editxml/)                                      │
    └──────────────────────────────────────────────────────┘
                                  ↓
    ┌──────────────────────────────────────────────────────┐
    │ 1-2. 動画から特徴量を抽出                            │
    │      python extract_video_features_parallel.py       │
    │      → data/processed/source_features/*.csv          │
    │      (音声 + 映像 + 時系列特徴量)                    │
    └──────────────────────────────────────────────────────┘
                                  ↓
    ┌──────────────────────────────────────────────────────┐
    │ 1-3. XMLからアクティブラベルを抽出                   │
    │      python extract_active_labels.py                 │
    │      → data/processed/active_labels/*.csv            │
    │      (採用/不採用の判定)                             │
    └──────────────────────────────────────────────────────┘
                                  ↓
    ┌──────────────────────────────────────────────────────┐
    │ 1-4. 時系列特徴量を追加                              │
    │      python scripts/add_temporal_features.py         │
    │      → data/processed/source_features/*_enhanced.csv │
    │      (移動平均、変化率、カットタイミング等)           │
    └──────────────────────────────────────────────────────┘
                                  ↓
    ┌──────────────────────────────────────────────────────┐
    │ 1-5. K-Fold用データを作成                            │
    │      python scripts/create_combined_data_for_kfold.py│
    │      → preprocessed_data/combined_sequences.npz      │
    │      (動画単位で分割、データリーク防止)               │
    └──────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│                    2. 学習フェーズ                                │
└─────────────────────────────────────────────────────────────────┘
                                  ↓
    ┌──────────────────────────────────────────────────────┐
    │ 2-1. K-Fold Cross Validationで学習                   │
    │      train_cut_selection_kfold_enhanced.bat          │
    │      → checkpoints_cut_selection_kfold_enhanced/     │
    │      (5-Fold、GroupKFold、データリーク防止)          │
    └──────────────────────────────────────────────────────┘
                                  ↓
    ┌──────────────────────────────────────────────────────┐
    │ 2-2. 学習状況をリアルタイム確認                      │
    │      ブラウザで view_training.html を開く            │
    │      (2秒ごとに自動更新される6つのグラフ)            │
    └──────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│                    3. 推論フェーズ                                │
└─────────────────────────────────────────────────────────────────┘
                                  ↓
    ┌──────────────────────────────────────────────────────┐
    │ 3-1. 新しい動画から特徴量を抽出                       │
    │      (inference_pipeline.py内で自動実行)             │
    └──────────────────────────────────────────────────────┘
                                  ↓
    ┌──────────────────────────────────────────────────────┐
    │ 3-2. モデルでカット位置を予測                        │
    │      run_inference.bat "video.mp4"                   │
    │      → outputs/inference_results/result.xml          │
    │      (2値分類 → 後処理でフィルタリング)              │
    └──────────────────────────────────────────────────────┘
                                  ↓
    ┌──────────────────────────────────────────────────────┐
    │ 3-3. Premiere Proで開く                              │
    │      result.xmlをインポート                          │
    └──────────────────────────────────────────────────────┘
```

---

## 📝 詳細な手順

### フェーズ1: データ準備

#### ステップ1-1: 編集済み動画とXMLを用意
```
editxml/
├── video1.mp4
├── video1.xml  (Premiere ProからエクスポートしたXML)
├── video2.mp4
└── video2.xml
```

**最低30本以上**の動画を用意することを推奨

#### ステップ1-2: 動画から特徴量を抽出
```bash
python -m src.data_preparation.extract_video_features_parallel \
    --video_dir videos \
    --output_dir data/processed/source_features \
    --n_jobs 4
```

**処理時間**: 5-10分/動画（10分の動画、GPU使用時）

**抽出される特徴量**:
- **音声特徴量（215次元）**:
  - 基本音声: RMS energy, VAD（発話検出）, 無音区間
  - 話者埋め込み: 192次元（pyannote.audio）
  - 音響特徴: MFCC（13次元）, pitch, spectral_centroid等
  - テキスト: Whisperによる音声認識フラグ
  
- **映像特徴量（522次元）**:
  - 基本映像: シーン変化, モーション, 顕著性マップ
  - 顔特徴量: 位置, サイズ, 表情（MediaPipe、10次元）
  - CLIP特徴量: 512次元の視覚的意味表現

**出力**: `data/processed/source_features/video1_features.csv`

#### ステップ1-3: XMLからアクティブラベルを抽出
```bash
python -m src.data_preparation.extract_active_labels \
    --xml_dir editxml \
    --feature_dir data/processed/source_features \
    --output_dir data/processed/active_labels
```

**出力**: `data/processed/active_labels/video1_active.csv`

**抽出される情報**:
- time: タイムスタンプ
- active: 採用（1）/ 不採用（0）

#### ステップ1-4: 時系列特徴量を追加
```bash
python scripts/add_temporal_features.py
```

**追加される特徴量（83個）**:
1. **移動統計量**: MA5, MA10, MA30, MA60, MA120, STD5, STD30, STD120
2. **変化率**: DIFF1, DIFF2, DIFF30
3. **カットタイミング**: time_since_prev, time_to_next, cut_duration, position_in_video, cut_density_10s
4. **CLIP類似度**: clip_sim_prev, clip_sim_next, clip_sim_mean5
5. **音声変化**: audio_change_score, silence_to_speech, speech_to_silence, speaker_change, pitch_change
6. **映像変化**: visual_motion_change, face_count_change, saliency_movement
7. **累積統計**: cumulative_position, cumulative_adoption_rate

**出力**: `data/processed/source_features/video1_features_enhanced.csv`

#### ステップ1-5: K-Fold用データを作成
```bash
python scripts/create_combined_data_for_kfold.py
```

**処理内容**:
1. 特徴量とアクティブラベルを時間ベースでマージ
2. シーケンス分割（長さ1000フレーム、オーバーラップ500）
3. **動画単位でグループ化**（GroupKFold用）
   - 67本の動画 → 289シーケンス
   - 同じ動画のシーケンスは必ず同じFoldに配置
4. 特徴量の正規化（StandardScaler）

**出力**:
- `preprocessed_data/combined_sequences_cut_selection_enhanced.npz` (289シーケンス)
- `preprocessed_data/audio_scaler_cut_selection_enhanced.pkl`
- `preprocessed_data/visual_scaler_cut_selection_enhanced.pkl`
- `preprocessed_data/temporal_scaler_cut_selection_enhanced.pkl`

---

### フェーズ2: 学習

#### ステップ2-1: K-Fold Cross Validationで学習
```bash
train_cut_selection_kfold_enhanced.bat
```

**モデルアーキテクチャ**:
- **入力**: 音声（235次元）+ 映像（543次元）+ 時系列（6次元）= **784次元**
- **埋め込み**: 各モダリティを256次元に変換
- **融合**: Gated Fusion（3モダリティを動的に重み付け）
- **エンコーダ**: Transformer Encoder（6層、8ヘッド、256次元）
- **出力**: Active判定（2クラス分類: 0=不採用、1=採用）

**学習設定**:
- K-Fold: 5分割（GroupKFold）
- エポック数: 50/Fold
- バッチサイズ: 16
- 学習率: 0.0001
- 損失関数: Focal Loss（alpha=0.5、gamma=2.0）
- Total Variation Loss: 0.02（時間的な滑らかさ）
- Adoption Penalty: 10.0（採用率を23%に近づける）
- Early Stopping: 15エポック
- Mixed Precision: 有効
- Random Seed: 42（再現性確保）

**学習時間**: 約2-3時間（250エポック = 50 × 5 Folds、GPU使用時）

**Early Stopping効果**:
- 平均収束: 7.4エポック
- 実際の学習: 37エポック（85%削減）

#### ステップ2-2: 学習状況をリアルタイム確認

ブラウザで `checkpoints_cut_selection_kfold_enhanced/view_training.html` を開く

**可視化される情報**（2秒ごとに自動更新）:

**全体進捗（kfold_realtime_progress.png）**:
1. F1スコアの推移（各Fold）
2. Validation Lossの推移（各Fold）
3. 現在のF1スコア（棒グラフ）
4. 進捗状況（テキスト）
5. 最良F1の推移（各Fold）
6. 採用/不採用の正確性（Recall & Specificity）

**各Fold詳細（fold_X/training_progress.png）**:
1. 損失関数（Train/Val Loss）
2. 損失の内訳（CE Loss vs TV Loss）
3. 分類性能（Accuracy & F1 Score）
4. Precision, Recall, Specificity
5. 最適閾値の推移
6. 予測の採用/不採用割合

**保存されるファイル**:
```
checkpoints_cut_selection_kfold_enhanced/
├── fold_1_best_model.pth          # Fold 1最良モデル (F1: 49.42%)
├── fold_2_best_model.pth
├── fold_3_best_model.pth
├── fold_4_best_model.pth
├── fold_5_best_model.pth
├── kfold_summary.csv              # 各Foldの結果サマリー
├── kfold_comparison.png           # Fold間の比較グラフ
├── kfold_realtime_progress.png    # リアルタイム進捗グラフ
├── inference_params.yaml          # 推論用パラメータ
└── view_training.html             # 訓練結果ビューア
```

---

### フェーズ3: 推論

#### ステップ3-1: 新しい動画でカット位置を予測
```bash
run_inference.bat "path\to\your_video.mp4"
```

**処理フロー**:
1. **特徴量抽出**: 動画から音声・映像・時系列特徴量を抽出
2. **特徴量正規化**: 学習時のScalerで正規化
3. **モデル推論**: Transformer Encoderで予測
   - 出力: 各フレームのActive確率（0〜1）
4. **後処理（クリップフィルタリング）**:
   - 最適閾値でActive判定（例: -0.558）
   - 最小継続時間フィルタ: 3.0秒未満のクリップを除外
   - ギャップ結合: 2.0秒以内の不採用区間を結合
   - 優先順位付け: Active確率順にソート
   - 合計時間制限: 目標90秒、最大150秒
5. **XML生成**: Premiere Pro用のXMLを生成

**推論時間**: 約5-10分/動画（特徴量抽出含む）

**出力**: `outputs/inference_results/result.xml`

#### ステップ3-2: Premiere Proで開く

生成されたXMLをPremiere Proで開くと、自動的にカット編集されたタイムラインが表示されます。

---

## 🔧 設定ファイル

### `configs/config_cut_selection_kfold_enhanced.yaml`
K-Fold学習設定

```yaml
# データ
data_path: 'preprocessed_data/combined_sequences_cut_selection_enhanced.npz'
audio_scaler_path: 'preprocessed_data/audio_scaler_cut_selection_enhanced.pkl'
visual_scaler_path: 'preprocessed_data/visual_scaler_cut_selection_enhanced.pkl'
temporal_scaler_path: 'preprocessed_data/temporal_scaler_cut_selection_enhanced.pkl'

# K-Fold設定
n_folds: 5
random_state: 42  # 再現性確保

# モデル設定
audio_features: 235
visual_features: 543
temporal_features: 6
d_model: 256
nhead: 8
num_encoder_layers: 6
dim_feedforward: 1024
dropout: 0.15

# 学習設定
batch_size: 16
num_epochs: 50
learning_rate: 0.0001
weight_decay: 0.0001
max_grad_norm: 1.0
use_amp: true  # Mixed Precision

# Early Stopping
early_stopping_patience: 15

# 損失関数
use_focal_loss: true
focal_alpha: 0.5
focal_gamma: 2.0
label_smoothing: 0.0
tv_weight: 0.02
adoption_penalty_weight: 10.0
target_adoption_rate: 0.23
```

### `checkpoints_cut_selection_kfold_enhanced/inference_params.yaml`
推論パラメータ（学習時に自動生成）

```yaml
# Fold 1の最適パラメータ
confidence_threshold: -0.558  # 最適閾値
min_clip_duration: 3.0        # 最小クリップ継続時間（秒）
max_gap_duration: 2.0         # ギャップ結合の最大長（秒）
target_duration: 90.0         # 目標合計時間（秒）
max_duration: 150.0           # 最大合計時間（秒）
```

---

## 📊 データフロー詳細

### 特徴量の次元数
- **音声**: 235次元
  - 基本音声: 4次元
  - 話者埋め込み: 192次元
  - 音響特徴: 16次元
  - テキスト: 3次元
  - 時系列: 20次元

- **映像**: 543次元
  - 基本映像: 10次元
  - CLIP: 512次元
  - 時系列: 21次元

- **時系列**: 6次元
  - カットタイミング: 6次元

- **合計**: 784次元

### シーケンスデータ
- **シーケンス長**: 1000フレーム（約100秒 @ 10FPS）
- **オーバーラップ**: 500フレーム
- **総データ**: 289シーケンス（67動画）
- **K-Fold**: 5分割（GroupKFold）
  - 各Foldで12-14動画を検証用に使用

### モデル出力
- **Active判定**: (batch, seq_len, 2)
  - クラス0: 不採用
  - クラス1: 採用
- **Confidence Score**: logits[1] - logits[0]
  - 正の値: 採用傾向
  - 負の値: 不採用傾向

---

## 💡 ヒント

### カット数を調整したい
`checkpoints_cut_selection_kfold_enhanced/inference_params.yaml` を編集：
- `confidence_threshold` を下げる → カット数が増える
- `confidence_threshold` を上げる → カット数が減る
- `target_duration` を変更 → 目標合計時間を調整

### 学習データを増やしたい
1. 新しい動画とXMLを `editxml/` に追加
2. ステップ1-2から再実行
3. **最低30本以上**を推奨、理想は50-100本

### 学習が進まない場合
- データ数を確認（最低30本以上推奨）
- GPU使用を確認（`nvidia-smi`）
- 設定を確認（`configs/config_cut_selection_kfold_enhanced.yaml`）
- Early Stoppingで早期に止まっていないか確認

### データリークを防ぐには
- **GroupKFold**を使用（同じ動画は同じFoldに）
- 各Foldで完全に未見のデータで評価
- アンサンブル評価は避ける（データが少ない場合）

---

## 🎯 性能指標

### 学習結果（K-Fold CV）
- **平均F1スコア**: 0.4230 ± 0.0575
- **最良F1スコア**: 0.4942（Fold 1、Epoch 4）
- **平均Recall**: 0.7610 ± 0.0519（採用カットを見逃さない）
- **平均Precision**: 0.2983 ± 0.0580（誤検出が多い）
- **最適閾値**: -0.558 (Fold 1)
- **学習時間**: 約2-3時間（250エポック、5 Folds、GPU使用時）

### 推論結果
- **推論時間**: 5~10分/動画（特徴量抽出含む）
- **カット数**: 約8〜12個のクリップ
- **出力動画長**: 約2分（90秒〜150秒）

### 目標達成状況
| 項目 | 目標 | 達成 | 状況 |
|------|------|------|------|
| F1スコア | 55% | 42.30% | ❌ 未達成 (-12.70pt) |
| Recall | 71% | 76.10% | ✅ 達成 (+5.10pt) |

---

## 📖 関連ドキュメント

- [README](../../README.md) - プロジェクト概要
- [QUICK_START](../QUICK_START.md) - クイックスタートガイド
- [FINAL_RESULTS](../FINAL_RESULTS.md) - 最終結果レポート
- [TRAINING_REPORT](../TRAINING_REPORT.md) - 詳細学習レポート
- [K_FOLD_CROSS_VALIDATION](../K_FOLD_CROSS_VALIDATION.md) - K-Fold CV詳細

---

**最終更新**: 2025-12-26  
**バージョン**: 3.0.0（K-Fold CV + 時系列特徴量版）

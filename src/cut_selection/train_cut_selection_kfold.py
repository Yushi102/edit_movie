...
class KFoldVisualizer:
    ...
    def plot_realtime_progress(self):
        ...
        fig.suptitle(f'K-Fold Cross Validation - Real-time Progress', fontsize=16, fontweight='bold')
        ...
        for fold_num in sorted(self.current_fold_data.keys()):
            ...
            if data['epoch']:
                ...
                progress_text += f"  Current F1: {current_f1:.4f}\n"
                progress_text += f"  Best F1: {max_f1:.4f}\n\n"
...
def validate(model, dataloader, criterion, device):
    ...
    precisions, recalls, thresholds = precision_recall_curve(all_labels, all_confidence_scores)
    ...
    logger.debug(f"Optimal threshold: {best_threshold:.4f} (F1={best_f1_threshold:.4f})")
    
    # Use OPTIMAL THRESHOLD for all metrics (not argmax)
    optimal_predictions = (all_confidence_scores >= best_threshold).astype(int)
...
def train_single_fold(fold, train_indices, val_indices, full_dataset, config, device, checkpoint_dir, visualizer):
    ...
    # ペナルティを均等に設定（Precision重視）
    # 採用を不採用とする間違い（False Negative）と不採用を採用とする間違い（False Positive）を同等にペナルティ
    ...
    # 不採用クラスの重みを3倍に調整（両方同じ倍率）
    weight_inactive = base_weight_inactive * 3.0
    ...
    logger.info(f"  🔥 Active class penalty: {weight_active/weight_inactive:.2f}x stronger (miss採用 = 大ペナルティ)")
    logger.info(f"  ⚠️  Both errors are penalized (False Positive & False Negative)")

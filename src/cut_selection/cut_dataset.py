"""
Dataset for cut selection training

Loads audio + visual features with active labels from source videos.
"""
import numpy as np
import torch
...
class CutSelectionDataset(Dataset):
    """
    Dataset for cut selection (Stage 1)
    
    Input: Audio + Visual features from source videos
    ...
    def __init__(self, sequences_npz: str):
        ...
        Args:
            sequences_npz: Path to .npz file with cut selection sequences
                          (created by scripts/create_cut_selection_data.py)
        """
        self.sequences_npz = sequences_npz
        
        logger.info(f"Loading cut selection data from {sequences_npz}")

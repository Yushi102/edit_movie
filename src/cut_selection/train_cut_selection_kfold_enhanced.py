...
def validate(model, dataloader, criterion, device):
    ...
    for i, (prec, rec, thresh) in enumerate(zip(precisions, recalls, thresholds)):
        # Only consider thresholds with Recall >= 80%
        if rec < min_recall:
            continue
        ...
        tn = np.sum((candidate_predictions == 0) & (all_labels == 0))
    ...
    # If no threshold satisfies Recall >= 80%, use the one with highest Recall
    if best_composite_score == 0.0:
        best_idx = np.argmax(recalls)
        best_threshold = thresholds[best_idx] if best_idx < len(thresholds) else 0.0
        best_recall = recalls[best_idx]
        logger.warning(f"⚠️  Could not achieve Recall >= {min_recall:.0%}, using threshold with Recall={best_recall:.2%}")
    else:
        logger.debug(f"Optimal threshold: {best_threshold:.4f} (F1={best_f1:.4f}, Specificity={best_specificity:.4f}, Recall={best_recall:.2%}, Composite={best_composite_score:.4f})")
    
    # Use OPTIMAL THRESHOLD for all metrics
    optimal_predictions = (all_confidence_scores_calibrated >= best_threshold).astype(int)
    
    # 🔒 FORCE ADOPTION RATE TO BE <= 80%
    # If predicted adoption rate exceeds 80%, clip it by changing lowest confidence predictions
    max_adoption_rate = 0.80
    current_adoption_rate = np.mean(optimal_predictions)

    
    if current_adoption_rate > max_adoption_rate:
        # Calculate how many predictions need to be changed from Active (1) to Inactive (0)
        num_active = np.sum(optimal_predictions == 1)
        max_allowed_active = int(len(optimal_predictions) * max_adoption_rate)
        num_to_flip = num_active - max_allowed_active
        
        if num_to_flip > 0:
            # Find indices where prediction is Active (1)
            active_indices = np.where(optimal_predictions == 1)[0]
            
            # Get confidence scores for these active predictions
            active_confidences = all_confidence_scores_calibrated[active_indices]
            
            # Sort by confidence (ascending) - flip the least confident ones
            sorted_indices = np.argsort(active_confidences)
            indices_to_flip = active_indices[sorted_indices[:num_to_flip]]
            
            # Flip predictions from Active to Inactive
            optimal_predictions[indices_to_flip] = 0
            
            logger.info(f"⚠️  Adoption rate clipped: {current_adoption_rate:.2%} → {np.mean(optimal_predictions):.2%} (flipped {num_to_flip} predictions)")
    
    # Calculate metrics using optimal threshold (with clipping applied)
...
def train_single_fold(fold, train_indices, val_indices, full_dataset, config, device, checkpoint_dir, visualizer):
    ...
    # Focal Lossが既に難しいサンプルに注目しているため、クラスウェイトは穏やかに設定
    ...
    logger.info(f"  Precision focus: Active 1.5x, Inactive 15x (EXTREME False Positive reduction)")
    ...
    criterion = CombinedCutSelectionLoss(
        ...
        target_adoption_rate=config.get('target_adoption_rate', 0.23),
        adoption_penalty_weight=config.get('adoption_penalty_weight', 0.5)
    )
